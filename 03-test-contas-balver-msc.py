'''
Testa se todas as contas do BAL_VER estão na MSC
'''
...
logging.info('Testando se todas as contas do BAL_VER estão na MSC...')
...
for conta_contabil in contas_balver:
    existe_msc = msc.query(f'conta_contabil == {conta_contabil}')
    if len(existe_msc) == 0:
        if inconsistencias.empty:
            inconsistencias = balver.query(f'conta_contabil == {conta_contabil}')
        else:
            inconsistencias = pd.concat([inconsistencias, balver.query(f'conta_contabil == {conta_contabil}')])
...
inconsistencias.to_excel(r'output\teste-contas-balver-msc.xlsx', sheet_name='dados', header=True, index=False)
